...
import {UpdateGiftById} from "@/actions/gift";
...
export default function UpdateExchangeGiftForm(prop: {
  setOpen: (open: boolean) => void;
  gift: GiftType;
}) {
  const {gift, setOpen} = prop;
  ...
  const onSubmit = async (values: z.infer<typeof AddGiftSchema>) => {
    ...
    startTransition(async () => {
      ...
      UpdateGiftById(gift.gift_id, values).then((data) => {
        setError(data.error);
        if (data.success) {
          setError("");
          setOpen(false);
